...
async fn disburse_interest() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (vault_address, vault_account) in vaults {
        ...
        // Use Pack trait method correctly for newer versions
        let mint_account_data = MintAccount::unpack(&serialized_mint_account_data).unwrap();

        let payer_is_mint_authority = mint_account_data.freeze_authority == Some(payer.pubkey());

        if payer_is_mint_authority {
            ...
            // Convert anchor_lang AccountMeta to solana_program AccountMeta
            let solana_accounts: Vec<solana_program::instruction::AccountMeta> = accounts
                .into_iter()
            ...
            let send_interest_instruction =
                Instruction::new_with_bytes(vault_program_id, &data, solana_accounts);
